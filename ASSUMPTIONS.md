...
- **Fixed ticker universe** (25 IDX large-caps, `.JK`) defined in `portfolio-app/data/universe.js` as two exports: `UNIVERSE_JK` (research — optimizer app + backtester) and `FORWARD_TEST_UNIVERSE_JK` (pinned — the live forward test, frozen at launch). This introduces **survivorship bias** — delisted/merged names are not represented, and neither list is point-in-time. The pin means the forward test's opportunity set is fixed by construction: editing `UNIVERSE_JK` changes what the optimizer app and backtester see, never what the 300 live streams trade. (The dashboard fetch additionally unions in any ticker still held in `portfolios.json`, so a removed name keeps being tracked.)
