...
**Backtester (local):**
```bash
cd backtest-portfolio && npm install && npm run fetch && npm run backtest && npm run dev
```
...
Weights reach the dashboard's `portfolios.json` either **by hand** (copy from the optimizer's Analytics tab) or **automatically** via the weekly rebalance Action ([`.github/workflows/weekly-rebalance.yml`](.github/workflows/weekly-rebalance.yml) → `portfolio-app/scripts/optimize.mjs`). See [`live-dashboard-portfolio/README.md`](live-dashboard-portfolio/README.md) for Vercel deploy and rebalance workflow.
...
| [ARCHITECTURE.md](ARCHITECTURE.md) | Monorepo structure, data flow across the apps (optimizer · backtester · dashboard), deployment & CI. |
