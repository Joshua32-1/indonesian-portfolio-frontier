...
It holds a **methodology matrix**: **60 streams = 10 configs × 6 strategy variants**. A config is `pert` (legacy Beta-PERT consensus, BL off) or Black-Litterman × prior `{cap, shrunk, equal}` × τ `{0.01, 0.03, 0.10}`. The stream `id` is `<base>@<configTag>` (configTag = `pert` or `bl-<prior>-t<NN>`, `NN = round(τ·100)`). The empty skeleton is regenerated by [`live-dashboard-portfolio/scripts/init-portfolios-matrix.mjs`](live-dashboard-portfolio/scripts/init-portfolios-matrix.mjs).
...
```jsonc
{
  ...
  "portfolios": [
    {
      "id":          "max-sharpe@bl-cap-t03", // unique <base>@<configTag>
    ...
    // 60 streams: bases {max-sharpe, min-var, tail-10/20/35/50}
