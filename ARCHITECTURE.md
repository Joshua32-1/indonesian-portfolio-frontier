...
│   │   └── optimize.mjs             # headless rebalance → appends to dashboard portfolios.json
│   ├── src/
│   │   ├── App.jsx                  # 4-tab UI
...
└── live-dashboard-portfolio/       # TRACKER — Vercel production
    ...
    │   └── portfolios.json          # weight history (manual or via optimize.mjs)
...
The bridge to the dashboard is `portfolios.json`, appended **either by hand** (the analyst reads weights off the optimizer's Analytics tab) **or automatically** by the weekly rebalance Action (`optimize.mjs` → appends a dated row per strategy). Separately, `refresh-views.yml` captures point-in-time analyst views weekly into `portfolio-app/data/view-history/` so the live strategy can later be κ-replayed.
