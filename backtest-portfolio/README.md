...
**The alpha signal is instead validated forward**, by the live dashboard (`live-dashboard-portfolio/`), which began tracking on **2026-06-30**. Point-in-time analyst views are now captured weekly (`portfolio-app/data/view-history/`) so a *true* full-strategy retro-backtest becomes possible once enough history accumulates.
